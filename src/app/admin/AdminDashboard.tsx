...
// Apibrėžti IŠORĖJE – taip React nekuria naujų komponentų kiekvieną re-render
type UpdateField = (path: string[], value: any) => void;
...
export function AdminDashboard({ initialContent, logoutAction }: { initialContent: SiteContent, logoutAction: () => void }) {
  ...
  // Pagalbinė funkcija: gauti reikšmę iš path
  const getVal = (path: string[]): string =>
    path.reduce((obj: any, key) => obj?.[key], content) ?? "";
      ...
      case "hero":
        return (
          <div className="space-y-6">
            ...
            <ImagePickerField
              label="Hero nuotrauka (Dešinėje pusėje)"
              value={getVal(["hero", "image"])}
              onClear={() => updateField(["hero", "image"], "")}
              onUpload={async (fd) => { const url = await uploadFile(fd); updateField(["hero", "image"], url); return url; }}
            />
            <div className="grid md:grid-cols-2 gap-6">
              <FieldInput label="Badge (Mažas tekstas viršuje)" value={getVal(["hero", "badge"])} onChange={(v) => updateField(["hero", "badge"], v)} />
              <FieldInput label="Išryškintas žodis (Pabraukimui)" value={getVal(["hero", "highlight"])} onChange={(v) => updateField(["hero", "highlight"], v)} />
            </div>
            <FieldInput label="Antraštė" value={getVal(["hero", "title"])} onChange={(v) => updateField(["hero", "title"], v)} />
            <FieldInput label="Aprašymas" value={getVal(["hero", "description"])} onChange={(v) => updateField(["hero", "description"], v)} multiline />
            <div className="grid md:grid-cols-2 gap-6">
              <FieldInput label="Pagrindinio mygtuko tekstas (CTA 1)" value={getVal(["hero", "primaryCta"])} onChange={(v) => updateField(["hero", "primaryCta"], v)} />
              <FieldInput label="Antrinio mygtuko tekstas (CTA 2)" value={getVal(["hero", "secondaryCta"])} onChange={(v) => updateField(["hero", "secondaryCta"], v)} />
            </div>
          </div>
        );
          ...
          <div className="space-y-6">
            ...
            {content.services.map((service, idx) => (
              <div key={idx} className="relative rounded-2xl border border-slate-200 bg-white p-6 shadow-sm mb-4">
                ...
                <div className="pr-8">
                  <FieldInput label="Paslaugos pavadinimas" value={service.title} onChange={(v) => updateField(["services", String(idx), "title"], v)} />
                  <FieldInput label="Trumpas aprašymas (kortelėje)" value={service.description} onChange={(v) => updateField(["services", String(idx), "description"], v)} multiline />
                  <FieldInput label="Pilnas aprašymas (paslaugos puslapyje)" value={service.fullDescription ?? ""} onChange={(v) => updateField(["services", String(idx), "fullDescription"], v)} multiline />
                  <FieldInput label="URL (slug, pvz. butu-valymas)" value={service.slug ?? ""} onChange={(v) => updateField(["services", String(idx), "slug"], v)} />
                  <ImagePickerField
                    label="Paslaugos nuotrauka"
                    value={service.image ?? ""}
                    onClear={() => updateField(["services", String(idx), "image"], "")}
                    onUpload={async (fd) => { const url = await uploadFile(fd); updateField(["services", String(idx), "image"], url); return url; }}
                  />
                  <div className="mt-4 border-t border-slate-100 pt-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button
                        onClick={() => {
                          const updated = JSON.parse(JSON.stringify(content.services));
                    ...
                    {(service.features || []).map((feature, fIdx) => (
                      <div key={fIdx} className="flex items-center gap-2 mb-2">
                        ...
                        <button
                          onClick={() => {
                            const updated = JSON.parse(JSON.stringify(content.services));
                            updated[idx].features = updated[idx].features.filter((_: string, i: number) => i !== fIdx);
                            updateField(["services"], updated);
                          }}
            ...
            {content.prices.map((price, idx) => (
              <div key={idx} className="relative rounded-2xl border border-slate-200 bg-white p-6 shadow-sm mb-4">
                ...
                <div className="grid md:grid-cols-2 gap-4 pr-8">
                  <FieldInput label="Paslaugos pavadinimas" value={price.name} onChange={(v) => updateField(["prices", String(idx), "name"], v)} />
                  <FieldInput label="Kaina (pvz. Nuo 55 €)" value={price.price} onChange={(v) => updateField(["prices", String(idx), "price"], v)} />
                </div>
                <FieldInput label="Papildoma informacija / Detalės" value={price.details} onChange={(v) => updateField(["prices", String(idx), "details"], v)} />
              </div>
            ))}
          </div>
        );
          ...
          <div className="space-y-6">
            ...
            {content.faq.map((item, idx) => (
              <div key={idx} className="relative rounded-2xl border border-slate-200 bg-white p-6 shadow-sm mb-4">
                ...
                <div className="pr-8">
                  <FieldInput label="Klausimas" value={item.question} onChange={(v) => updateField(["faq", String(idx), "question"], v)} />
                  <FieldInput label="Atsakymas" value={item.answer} onChange={(v) => updateField(["faq", String(idx), "answer"], v)} multiline />
                </div>
              </div>
            ))}
          </div>
      ...
      case "about":
        return (
          <div className="space-y-6">
            ...
            <ImagePickerField
              label="Sekcijos nuotrauka"
              value={getVal(["about", "image"])}
              onClear={() => updateField(["about", "image"], "")}
              onUpload={async (fd) => { const url = await uploadFile(fd); updateField(["about", "image"], url); return url; }}
            />
            <FieldInput label="Antraštė" value={getVal(["about", "title"])} onChange={(v) => updateField(["about", "title"], v)} />
            <FieldInput label="Aprašymas" value={getVal(["about", "description"])} onChange={(v) => updateField(["about", "description"], v)} multiline />
      ...
      case "company":
        return (
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <FieldInput label="Įmonės / Puslapio pavadinimas" value={getVal(["company", "name"])} onChange={(v) => updateField(["company", "name"], v)} />
              <FieldInput label="Miestas" value={getVal(["company", "city"])} onChange={(v) => updateField(["company", "city"], v)} />
              <FieldInput label="Adresas" value={getVal(["company", "address"])} onChange={(v) => updateField(["company", "address"], v)} />
              <FieldInput label="Aptarnavimo zona" value={getVal(["company", "serviceArea"])} onChange={(v) => updateField(["company", "serviceArea"], v)} />
              <FieldInput label="Telefono numeris rodymui" value={getVal(["company", "displayPhone"])} onChange={(v) => updateField(["company", "displayPhone"], v)} />
              <FieldInput label="Telefonas nuorodai" value={getVal(["company", "phone"])} onChange={(v) => updateField(["company", "phone"], v)} />
              <FieldInput label="El. paštas" value={getVal(["company", "email"])} onChange={(v) => updateField(["company", "email"], v)} />
              <FieldInput label="Facebook puslapio nuoroda" value={getVal(["company", "facebook"])} onChange={(v) => updateField(["company", "facebook"], v)} />
            </div>
            <div className="mt-8 border-t border-slate-200 pt-8">
              <h3 className="text-lg font-bold text-slate-900 mb-4">Kontaktų sekcija apačioje</h3>
              <FieldInput label="Antraštė" value={getVal(["contact", "title"])} onChange={(v) => updateField(["contact", "title"], v)} />
              <FieldInput label="Aprašymas" value={getVal(["contact", "description"])} onChange={(v) => updateField(["contact", "description"], v)} multiline />
              <div className="grid md:grid-cols-2 gap-6">
                <FieldInput label="Darbo laikas" value={getVal(["contact", "hours"])} onChange={(v) => updateField(["contact", "hours"], v)} />
                <FieldInput label="Skambučio mygtuko tekstas" value={getVal(["contact", "callLabel"])} onChange={(v) => updateField(["contact", "callLabel"], v)} />
              </div>
            </div>
          </div>
        );
      ...
      case "seo":
        return (
          <div className="space-y-6">
            ...
            <FieldInput label="Puslapio Pavadinimas (Title)" value={getVal(["seo", "title"])} onChange={(v) => updateField(["seo", "title"], v)} />
            <FieldInput label="Aprašymas (Meta description)" value={getVal(["seo", "description"])} onChange={(v) => updateField(["seo", "description"], v)} multiline />
            <div className="mt-6">
              <div className="flex items-center justify-between mb-4">
                ...
                {content.seo.keywords.map((kw, idx) => (
                  <div key={idx} className="flex items-center gap-2 rounded-full bg-slate-100 pl-4 pr-1 py-1">
                    <input
                      ...
                      value={kw}
                      onChange={(e) => {
                        const updated = [...content.seo.keywords];
                        updated[idx] = e.target.value;
                        updateField(["seo", "keywords"], updated);
                      }}
